...
#if UNITY_EDITOR
/*
Description: Tool to place objects in a 3D array. This works similar to 3DS Max array tool.
...
public class CArrayTool : EditorWindow
{
    ...
    */
    ...
    {
        CArrayTool window = (CArrayTool)EditorWindow.GetWindow(typeof(CArrayTool)); //Create an editor window
    ...
    */
    private void OnGUI()
    {
        ...
        else //If we want to palce elements
        {
        ...
        if (m_isCopyingElement == true)
        {
            ...
            if (m_objectToCopy != null)
            {
                ...
                {
                    //If the object we want to copy is not a prefab
                    if (PrefabUtility.GetPrefabType(m_objectToCopy) == PrefabType.None)
                    {
                        //Display error message
                        GUILayout.Label(M_LABEL_ERROR_OBJECT_NOT_PREFAB);
                    }
                    else //If the object we want to copy is a prefab
                    {
                ...
                else//If its not copying as instance of prefab
                {
                    //If the button is press
                    if (GUILayout.Button(M_BUTTON_COPY))
                    {
        ...
        else //If we want to place objects in the scene
        {
            //Display the corresponding fields
    ...
    /*
    Description: Display in the editor window the options to copy an object.
    Creator: Alvaro Chavez Mixco
    Creation Date: Sunday, January 29, 2017
    */
    ...
    /*
    Description: Display in the editor window the options to place an object.
    Creator: Alvaro Chavez Mixco
    Creation Date: Sunday, January 29, 2017
    */
    private void DisplayPlaceElementsFields()
    {
        ...
        if (m_getElementsFromParent == true)
        {
            ...
            if (m_parentObject != null && (m_parentObject != m_previousParentObject))
            {
                    ...
                    m_parentObject.transform.childCount != m_previousParentObjectchildCount)
                {
                    ...
                    GameObject[] childrenObjects = CUtilityGame.GetChildrenGameObjectFromParent(m_parentObject);

                    //Convert the array of children into a 3D array so that it can be used with the editor
                    m_listObjectsToPlace = CUtilityMath.Convert1DArrayGameObjectsTo3DArray(childrenObjects, m_arrayDimensions);
        ...
        else//If we are going to manually set all the objects that will be placed
        {
    ...
    /*
    Description: Display in the editor window the options to set the properties of an array.
    Creator: Alvaro Chavez Mixco
    Creation Date: Sunday, January 29, 2017
    */
    ...
    */
    private void CalculateIncrementalProperties()
    {
        ...
        if (m_isIncrementalArray == false)
        {
            ...
            m_incrementalDistance = CUtilityMath.GetValuesBetweenElements(m_totalDistance, m_arrayDimensions);

            //Get the angle between objects according to size of array
            m_incrementalAngle = CUtilityMath.GetValuesBetweenElements(m_totalAngle, m_arrayDimensions);

            //Get the scale between objects according to size of array
            m_incrementalScale = CUtilityMath.GetValuesBetweenElements(m_totalScale, m_arrayDimensions);
        }
    }
