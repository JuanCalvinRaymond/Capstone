...
Automatically toggles a post effect to depict the paused state of the game.
Creator: Charlotte C. Brown
*/
public class CPausedPostEffect : MonoBehaviour
{
    private const string M_UNIFORM_EFFECT_STRENGTH_NAME = "u_effectStrength";
    private Material m_postProcessMaterial;

	[Range(0.0f, 1.0f)]
    public float m_effectAmount = 1.0f;
    ...
    {
        CGameManager gameManager = CGameManager.PInstanceGameManager;

        // If the game is paused, apply the paused post-effect.
        if (gameManager != null)
        {
            if (gameManager.PGameState == EGameStates.Paused)
            {
